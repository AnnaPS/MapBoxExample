...
class MarkerActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun generateList() {
        madridPlaces.add(
            CityPlaces("40.36902428128397","-3.710212677306876",
                "Espacio de Igualdad Berta Cáceres",IGUALDAD)
        )
        madridPlaces.add(
            CityPlaces("40.46155641637237","-3.6501714580414126",
                "Espacio de Igualdad Carme Chacón",IGUALDAD)
        )
        madridPlaces.add(
            CityPlaces("40.42591696701239","-3.6929960000180335",
                "Audiencia Nacional. Sala de lo Penal",JUSTICIA)
        )
        madridPlaces.add(
            CityPlaces("40.43745442601165","-3.6752517217446106",
        ...
        madridPlaces.forEach {
            ...
            when(it.typePlace){
                EDUCACION ->{
                    ...
                    val marker = MarkerView(LatLng(it.lat.toDouble(),it.lag.toDouble()),customView)
                    marker?.let {marker ->
